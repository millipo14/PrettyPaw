...
const createArrow = (className = 'arrow-up', { hover = true } = {}) => {
    ...
    button.innerHTML = `<svg class="${className}_svg" width="16" height="16" viewBox="0 0 16 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
...
`;
    ...
    .${className}{
            ...
            box-shadow: 0 4px 4px 0 rgba(49, 33, 1, 0.15);
            border-radius: 50%;
            width: 50px;
            height: 50px;
...
export const initScrollTopButton = (className, options) => {
    ...
    const showElementScrollPosition = () => {
        const scrollPosition = window.scrollY || document.documentElement.scrollTop
        arrow.style.display = scrollPosition > window.innerHeight / 2 ? 'flex' : 'none'
    }

    window.addEventListener('scroll', debounce(showElementScrollPosition, 100))
}